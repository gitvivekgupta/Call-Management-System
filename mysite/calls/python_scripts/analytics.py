...
def analyse(path_file):
	...
	with open(path_file) as csv_ivr:
		...
		plt.figure(1)
		plt.hist(Missed_calls,np.arange(0,max(Missed_calls)+20,10),histtype = 'bar')
		plt.xlabel('Duration')
		plt.ylabel('No. of calls')
		plt.title('Missed calls in ivr file')
		plt.plot()
		plt.savefig(os.path.join(settings.BASE_DIR, 'media/Missed_calls.jpeg'))
		
		plt.figure(2)
		plt.hist(Connected_calls,np.arange(0,max(Connected_calls)+20,10),histtype = 'bar')
		plt.xlabel('Duration')
		plt.ylabel('No. of calls')
		plt.title('Connected calls in ivr file')
		plt.plot()
		plt.savefig(os.path.join(settings.BASE_DIR, 'media/Connected_calls.jpeg'))

		plt.figure(3)
		plt.hist(Voicemail,np.arange(0,max(Voicemail)+20,10),histtype = 'bar')
		plt.xlabel('Duration')
		plt.ylabel('No. of calls')
		plt.title('Voicemails in ivr file')
		plt.plot()
		plt.savefig(os.path.join(settings.BASE_DIR, 'media/Voicemail.jpeg'))

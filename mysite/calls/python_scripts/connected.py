...
def connected_calls(ivr_file,mobile_file):
	...
	with open(ivr_file,'r') as ivr_record_file:
		...
		dump_redundancy_set = {} #Dict is used to avoid the overlapping values in logfile and it is hashed.
		checked_entry = set({}) #set is used to avoid the overlapping values in logfile and it is hashed.
		...
		for ivr_file_entry in csv_ivr_file:
			...
			if ivr_file_entry[3] == 'Missed' and ivr_file_entry[4] not in checked_entry and notes[0] != 'Spam' and notes[0] != 'NR' and notes[0] !='spam': 
				...
				dump_redundancy_set.update(missed_calls(ivr_file_entry,mobile_file))
